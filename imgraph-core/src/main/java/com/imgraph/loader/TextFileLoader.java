...
public class TextFileLoader implements ResponseProcessor {
	...
	public static int loadBlock(List<LoadVertexInfo> verticesInfo) {
		...
		for (LoadVertexInfo loadVertex : verticesInfo) {
			...
			vertex = (ImgVertex) graph.retrieveRawCell(loadVertex.getVertexId());
			
			if (vertex == null) {
				vertex = new ImgVertex(loadVertex.getVertexId(), null, false);
	...
	private static void processVertexInfo(ImgraphGraph graph, List<LoadVertexInfo> verticesInfo) {
	...
	public static void singleProcessLoad(ImgraphGraph graph, String fileName) throws Exception {
		BigTextFile file = null;
		Date startDate, endDate;
